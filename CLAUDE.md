...
### セッションを開いたら、自分の名前にホストを入れる

**`[確定]` 2026-09-03**（利用者の指摘「**本人ができるのであれば、本人が書き換えるのがよい**」）。

**エージェントの名前は既定だと `takibi-0e` のように機械の区別が入らない。**
**同じプロジェクトが複数の機械にある**（`takibi@raytrek` / `takibi@zv` / `takibi@macbook`）ので、
**名前だけでは送り先を決められない。**

```sh
# **そのエージェントの Claude Code の入力欄で 1 回だけ打つ。**
# **綴りは <作業ディレクトリ名>.<ホストの通称>**
/rename takibi.raytrek
```

| | |
|---|---|
| **綴り** | **`<作業ディレクトリ名>.<ホストの通称>`**（`takibi-wt-1.raytrek` / `primadoc.zv`） |
| **`[取消]` `@` は使えない** | **`ListAgents` には出るのに、`SendMessage` の宛先にできない**（`@` が「チームの区切り」として読まれる / 下の `[実測]`） |
| **ホストの通称** | `raytrek` / `vaio` / `zv` / `macbook`（**機械の `hostname` ではなく、この通称**） |
...
- **`[実測]` 2026-09-03**: `/rename` は**即時に反映**され、
  ...
  **他のセッションの一覧にもすぐ出る**
- **`[取消]` 「そのまま宛先にできる」は誤りだった**（同日 / 複数のセッションからの指摘で判明）。
  **`@` を入れると `SendMessage` が
  `to must be a bare teammate name — there is only one team per session` で弾かれる。**
  **一覧に出ることと、宛先にできることは別だった** — **一覧で確かめて「送れる」と読んだのが誤り。**
  **`.` は通る**（`zzz-test.raytrek` へ実際に送って確認）。長さと使える字の上限は**公開文書に無い**
- **やる契機は「セッションを開いたとき」。**既に走っているセッションは、気づいたときに直す
- **`[要注意]` 改名すると、旧名では呼べなくなる。****「名前は鍵ではない」はここでは効かない**
  （`[実測]` 2026-09-03: 改名後に旧名へ送ると `No agent named ... is reachable`）。
  **直前までやり取りしていた相手には、改名したことを 1 通で名乗り直す** —
  **気づくのは、相手が返そうとした瞬間**になる
- **`[要注意]` 「送れた」は向きで別。****こちらから A へ送れた ≠ A からこちらへ送れる。**
  **確かめるなら、自分から 1 通投げる**（`[実測]` 2026-09-03 / 指摘は `im-viewer-wt-2`）

**なぜ本人がやるのか** — **ホストと作業場所を知っているのは本人だけ。**
**外から名簿を作ると、必ず古くなる。**
...
# **エージェントは名乗って始める**（上の「自分の名前にホストを入れる」）
claude -n <project>.<ホストの通称>
```
...
**`[試験中]` 2026-09-04**（利用者の指示。`im-viewer` / `takibi` / `primadoc` に続いて **rondine にも配布**）。
**手順の全文は `~/dev/takibi/docs/ops/handover.md`。**

**70% を超えると `UserPromptSubmit` hook が知らせる**（`~/dev/takibi/setup/context-warn.sh`。
**機械ごとの設定**として `.claude/settings.local.json` に置かれており、**リポジトリには載せない**）。
**知らせであって指示ではない** — **切るのは区切りがついたときだけ**
（**コミット済み・push 済み / 利用者への返答待ちが無い / 他のエージェントへの依頼が宙に浮いていない**）。

```sh
~/dev/takibi/setup/context-usage.sh   # ① 自分で測るならこれ（hook を待たなくてよい）
# ② ~/.claude/handover/<作業場所>.md に書く
#    いま何をしていたか / 次の一手 / 触っているブランチとファイル /
#    利用者との約束 / **他の機械に居る相手**（zv・vaio・macbook・他の worktree の専任）
~/dev/takibi/setup/handover.sh clear  # ③ 送る（引き継ぎが無ければ送らない）
```

- **読む側は自動**（`SessionStart` hook）。**新しい会話には引き継ぎが勝手に入り、
  最初の 1 通も自動で投げられる**ので、**`/clear` の先でも作業が止まらない**
- **最初の返答は 📋 の 4 行で始める**（注入の末尾に書式が出る）。
  **注入は画面に 1 文字も出ない**ので、**書かないと利用者からは「何も引き継がれていない」ように見える**
- **`[要注意]` 80% では遅い。****引き継ぎを書く作業そのものが context を使う**
- **`[要注意]` 権限で止められたら迂回しない。**利用者に「`/clear` を打ってください」と伝える
- **読んだ引き継ぎは消す**（注入の末尾に `rm` が出る。残すと次に古いものを読む）
- **引き継ぎはリポジトリの外に置く** — repo を汚さず、**`/clear` はファイルを消さない**
- **`[要注意]` 効き始めるのは、そのセッションが次に `/clear` するか `claude` を起動し直したときから**
  （`[実測]` 2026-09-04。**走っているセッションは設定を読み直さない**）
- **rondine は 3 段（`raytrek` → `zv` → `vaio`）を跨ぐので、引き継ぎには
  「どの機械で何を確かめたか」を必ず書く** — **機械が違えば結果が違う**（上の「開発機の使い分け」）
