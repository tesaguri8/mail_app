...
## 5.5 鍵のバックアップ・復元（重要・データ消失対策）

- **メール本体は再取得可能**: サーバー(IMAP)が源なので、ローカルDB消失は再同期で回復（致命的でない）。
- **失うと困るのは鍵とアプリ独自データ**:
  - SQLCipher DBキー / 保護領域の鍵（Lv1共有・Lv2本人鍵）/ TSG One トークン（keyring 保管＝端末側）。
  - **keyring の鍵を失うと、暗号化された過去の保護メッセージが復号不能**になる。
- **対策**: **鍵レジストリ（鍵ID→鍵素材）と保護領域鍵を、ユーザーのパスフレーズで暗号化してエクスポート**し、別端末で復元できるようにする。
- **任意**: ローカルDB＋`media` の暗号化バックアップ（サーバー保持期間より古いメールやオフライン用）。
- データルートを別マシンへ移しても、**鍵がなければ復号できない**点を UI で明示（[§1.5](#15-容量と分離再配置できるデータルート) と整合）。
