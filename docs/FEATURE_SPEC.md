...
#### 印刷（**実装済み**）
- メール 1 通を印刷する（本文表示のヘッダのプリンタアイコン、または会話バブルの右クリックメニュー）。プリンタのほか、ダイアログから「PDF として保存」も選べる。
- 版面は**非表示 iframe の中に専用の CSS で組む**（`components/PrintMail.tsx`）。アプリ本体の暗い配色や全面背景画像を持ち込まず、紙は白地・黒文字。本文の描画は画面と同じ `HtmlText` を使うので、サニタイズ規則や `cid:` 画像の解決は共通。
- 差出人・宛先・Cc・日時・添付ファイル名をヘッダに載せ、本文に埋め込まれない画像は本文の後ろに並べる（画面表示と同じ規準）。画像の読み込みを待ってから印刷ダイアログを開く（最大 5 秒）。
- スレッド全体の印刷は後続。
...
- **保存先**: `%APPDATA%\tesaguri.rondine.dev\media\backgrounds\`（[DATA_STORAGE.md](DATA_STORAGE.md)）。
...
- **核ルール**: 保護領域は「人には実値・AIには伏字」。Rondine の全AI機能はAIへ渡す前に伏字へ置換。
- **表示トグル（伏字⇄実値）**: 本文を伏字／実値で切替。**送信前に「相手・AIに渡る姿（伏字）」を目視確認**でき、漏れなく安心して送信できる。
- **受信**: 対応クライアントは復号インライン表示／非対応は PDF フォールバックをパスワードで開ける。
- **条件生成＆自動追記**: 保護領域が**無いメールは PDF/暗号を付けず普通に送信**。PDF を付ける時は**ランダムパスワードで暗号化し、暗証番号を本文へ自動追記**（送信前プレビューで確認）。
