...
Rondineの計画・設計ドキュメント。プロジェクトは **実装中（アルファ・0.1.0-alpha.1）** であり、技術スタックは **Primadoc 同等**（デスクトップ = Tauri 2 + Rust／モバイル = Expo / React Native）を採用する。デスクトップのコア機能（メール／住所録／検索／タグ・フィルタ）まで実装済みで、カレンダー・AI・SNS 統合・モバイルは未着手（計画）。
