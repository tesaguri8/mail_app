...
## チップ（トークン）UI（実装済み）
各宛先を**チップ（トークン）として区切って表示**し、末尾の入力欄で新規追加する。全員返信で Cc に
複数アドレスが流し込まれても、どこからどこまでが 1 件かが明確になり、末尾に別アドレスを確実に追加できる
（従来は末尾カンマ無しの塊で、直前のアドレスと繋がって別宛先を追加できない問題があった）。

- **契約は不変**: 親 Compose とは従来どおり `value: string`（カンマ区切り）/ `onChange` でやり取りする。
  RecipientInput 内部で「最後のカンマ以降＝編集中の下書き、それより前＝確定チップ」と解釈して描画する。
- **事前入力の全件チップ化**: 返信の宛先や下書き復元など事前入力は Compose の `init` で末尾 `", "` 止め
  （`committed()`）にして渡し、全件をチップ表示にする。
- **操作**: Enter / カンマ / セミコロン / 改行（貼り付け含む）で確定、`×` で削除、空欄 Backspace で直前チップを
  入力欄へ戻して編集、チップ本体クリックで編集。重複アドレスは追加しない。不正形式（`@`欠落等）は
  琥珀色で警告表示（送信自体は止めない）。IME 変換確定の Enter は誤確定しないようガード。
