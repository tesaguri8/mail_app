...
### 8.5 差出人単位の迷惑扱い（`spam_senders`）※実装済み

「このアドレスを迷惑にしたら、**同じ差出人の他のメールも全部迷惑（spam）へ**」という決定的（統計に依らない）ルール。**ブロック（受信拒否）ではなく迷惑メール扱い**（`is_junk=1`・学習と「非迷惑に戻す」が効く）に寄せる。ベイズ学習（§7）と併走し、学習が育つ前でも確実に効く。
