...
- **常駐性**: 「最前面に固定（always-on-top）」をトグルで提供。小さくしてもデスクトップ上に残せる（`TitleBar.tsx` の Pin トグルで実装済み）。
- **2 つの表示モード（計画・未実装）**: ウィンドウサイズに応じて自動的に切り替える構想（下記）。**現状は単一のダッシュボード（`Home.tsx`）のみで、ウィンドウサイズしきい値による自動切替・ウィジェット化は未実装。**

| モード | 条件（目安） | 表示内容 |
|--------|-------------|---------|
| **ダッシュボード**（実装済み） | 通常〜大サイズ | 背景画像 + 概要パネル（未読・新着・次の予定 等）+ ナビゲーション |
| **ウィジェット（コンパクト）**（計画・未実装） | 小サイズ（しきい値以下） | 背景画像 + 大きな時計・日付。最小限のバッジ（未読数・次の予定）のみ |
...
## 2.5 ウィジェット（コンパクト）モード ※計画（未実装）

> **未実装。** 以下は将来構想であり、現状の `Home.tsx` は固定のダッシュボード 1 種のみを描画する。ウィンドウサイズしきい値による自動切替・置き時計化のロジックは未実装。
...
> **実装との差分（重要）**: 下記の縦型グローバルサイドバーは**未実装**。現状のアプリ内ナビゲーションは、フレームレスの**タイトルバー上のアイコン列**（`TitleBar.tsx`）で行う：**ホーム / メール / 住所録 / カレンダー / 設定** ＋ 背景切替・言語切替・最前面固定（Pin）。
> コード上の「サイドバー」は、メール画面（`MailboxView.tsx`）の**リサイズ可能なメール一覧ペイン**（`Ctrl+S` で表示切替・ドラッグで幅可変）を指す。**ダークモード切替トグルは未実装（出荷していない）。** 以下の構成・仕様は将来検討用の参考として残す。
